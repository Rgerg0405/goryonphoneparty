...
export function useGameLogic(code: string | undefined, playerId: string, username: string, avatar: string) {
  ...
  const startGame = useCallback(async () => {
    ...
    // ===== AI PROMPT mode: AI generálja a kezdő mondatokat, ugorjuk a writing step-et =====
    if (g.settings.gameMode === 'ai-prompt') {
      try {
        const { data, error: fnErr } = await supabase.functions.invoke('ai-generate-prompts', {
          body: { count: order.length },
        });
        const fallback = [
          'Egy zsiráf gördeszkázik a Holdon',
          'Sajtszendvics atombomba',
          'Egy macska űrhajós lett',
          'Pizza esik az égből',
          'Tánciskola dinoszauruszoknak',
          'Egy mókus pólókereskedő',
          'Robot kávézó a Marson',
          'Szilveszter egy hangyabolyban',
        ];
        const prompts: string[] = (data?.prompts && Array.isArray(data.prompts) && data.prompts.length > 0)
          ? data.prompts
          : fallback.slice(0, order.length);

        // Insert AI prompt as step 0 text entry for every chain
        for (let i = 0; i < order.length; i++) {
          ...
          await supabase.from('game_entries').insert({
            ...
            step: 0,
            player_id: pid,
            player_name: player?.username || 'AI',
            entry_type: 'text',
            content: prompts[i % prompts.length] || fallback[i % fallback.length],
          });
        }
        if (fnErr) {
          toast({ title: 'AI hiba', description: 'Fallback promptokat használok.' });
        }
      } catch (e) {
        toast({ title: 'AI hiba', description: 'Nem sikerült promptokat generálni.' });
      }

      // Jump directly to drawing step (step 1)
      const nextPhase: GamePhase = 'drawing';
      const timeForPhase = g.settings.drawTime;
      channelRef.current?.send({
        type: 'broadcast',
        event: 'game:phase',
        payload: {
      ...
      submissionsRef.current = new Set();
      const chainIndex = getAssignedChainIndex(order, playerId, 1);
      const currentContent = await loadAssignmentContent(g.partyId, g.sessionNumber, chainIndex, 0, nextPhase);
